...
# Удаляем обработанный zip из репозитория через Contents API (атомарно).
# [skip ci] не даёт этому же удалению повторно запустить сборку.
function Cleanup-RepoFile {
    try {
        $blobSha = (git rev-parse "HEAD:$file" 2>$null)
        if ($blobSha) {
            $headers = @{
